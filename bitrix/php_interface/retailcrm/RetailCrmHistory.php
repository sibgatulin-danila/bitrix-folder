...
class RetailCrmHistory {
    ...
    public static $CRM_SITE = array('www-poisondrop-ru' => 's1', 'shop-top-leybl' => 's2', 'ip-migunov' => 's3');
    ...
    public static function orderHistory() {
        ...
        while (true) {
            ...
            $GLOBALS['RETAIL_CRM_HISTORY'] = true;
            //orders with changes
//            $orderDie = array('23861', '119481', '119531', '119567', '119620', '119665', '119716', '33485', '33095', '33096', '119804', '116873', '116791', '119580', '119843', '119904', '107180', '121143', '121257', '121308', '40738', '123679', '123573', '123869', '123919', '124001', '124103', '124253', '123338', '123492', '92045', '124845', '124002', '124614', '125687', '123386', '27576', '125881', '126761', '127724', '127563', '112386', '130509', '126757');
//            $orderDie = array('132497','132870','132497','132208','132232');
            foreach ($orders as $order) {
//                $orderDump = var_export($order, true);
//                file_put_contents('/var/www/poisondrop.ru/log/hh.log', $order['id'] . PHP_EOL, FILE_APPEND | LOCK_EX);
//                file_put_contents('/var/www/poisondrop.ru/log/fh.log', $orderDump . PHP_EOL, FILE_APPEND | LOCK_EX);
//                self::write_logs_order($order['id'], $order);
//                if (in_array($order['id'], $orderDie)) {
//                    continue;
//                }
                ...
                if (isset($order['externalId'])) {
                    ...
                    if ($order['externalId'] && is_numeric($order['externalId'])) {
                        try {
                            $rsOrder = \Bitrix\Sale\Internals\OrderTable::getList(['filter' => ['ID' => $order['externalId']], 'select' => ['ID', 'LID']]);
                            if ($arOrder = $rsOrder->fetch()) {
                                if (self::$CRM_SITE[$order['site']] != $arOrder['LID']) {
                                    if (self::changeSiteByOrder($order['externalId'], self::$CRM_SITE[$order['site']])) {
                                        file_put_contents('/var/www/poisondrop.ru/log/cou.log', $order['externalId'] . '- LID сайта обновлен<br>', FILE_APPEND | LOCK_EX);
                                    } else {
                                        file_put_contents('/var/www/poisondrop.ru/log/cou.log', $order['externalId'] . '- LID сайта не обновлен<br>', FILE_APPEND | LOCK_EX);
                                    }
                                }
                            }

                            $newOrder = Bitrix\Sale\Order::load($order['externalId']);
//                            if (!empty($newOrder) && method_exists($newOrder, 'getField')) {
//                                if ($newOrder->getField('PAYED') == 'Y' && empty($newOrder->getField('DATE_PAYED'))) {
//                                    $newOrder->setFieldNoDemand("PAYED", 'N');
//                                    $newOrder->save();
//                                }
//                            }
                        } catch (Bitrix\Main\ArgumentNullException $e) {
...
//                    $orderOrderDump = var_export($order, true);
//                    $orderDump = var_export($newOrder, true);
//                    file_put_contents('/var/www/poisondrop.ru/log/hh.log', $orderDump . PHP_EOL . '66666666' . PHP_EOL .  $orderOrderDump . PHP_EOL, FILE_APPEND | LOCK_EX);
                    if ($newOrder === null) {
                    ...
                    if (!$fUserId) {
                        if (empty($order['customer']['externalId'])) {
                            $fUserId = Bitrix\Sale\Fuser::getIdByUserId($newOrder->getField('USER_ID'));
                        } else {
    ...
    private static function orderSave($order) {
        try {
            ...
            self::write_logs_work('id', $order);
            self::write_logs_work('id', $exception->getMessage());
    ...
    public static function write_logs_work($name, $attributs) {
        $html = '/////get array' . PHP_EOL . $name . PHP_EOL . var_export($attributs, true);
        file_put_contents('/var/www/poisondrop.ru/html/import/order_die.txt', $html, FILE_APPEND | LOCK_EX);
    }

    public static function write_logs_order($name, $attributs) {
        $html = '/////get array' . PHP_EOL . $name . PHP_EOL . var_export($attributs, true);
        file_put_contents('/var/www/poisondrop.ru/html/import/order_logs.txt', $html, FILE_APPEND | LOCK_EX);
    }

    public static function write_logs_error($name, $attributs) {
        $html = '/////get array' . PHP_EOL . $name . PHP_EOL . var_export($attributs, true);
        file_put_contents('/var/www/poisondrop.ru/html/import/error_crm_update_order.txt', $html, FILE_APPEND | LOCK_EX);
    }

    public static function changeSiteByOrder($orderId, $siteId) {
        if (!\Bitrix\Main\Loader::includeModule('sale')) {
            return false;
        }
        $res = \Bitrix\Sale\Internals\OrderTable::update($orderId, ['LID' => $siteId]);
        if (!$res->isSuccess()) {
            write_logs_error($orderId, $res->getErrorMessages());
            return false;
        }
        return true;
    }
