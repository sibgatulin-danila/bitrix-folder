...
'PROPERTY_SIZE_RINGS.NAME'
            ...
            do {
                ...
                while ($product = $dbResProducts->GetNext()) {
                               //file_put_contents(__DIR__.'/testpoc.log', var_export($product,true),FILE_APPEND);
                               file_put_contents(__DIR__.'/testPic.log', var_export($product['PROPERTY_PHOTOGALLERY_VALUE'],true),FILE_APPEND);
                            if($product['PROPERTY_PHOTOGALLERY_VALUE']){
                                $arrrSelect = Array("ID", "NAME", 'IBLOCK_SECTION_ID','PROPERTY_REAL_PICTURE');
                                $arrrFilter = Array("IBLOCK_ID"=>7, "SECTION_ID"=>$product['PROPERTY_PHOTOGALLERY_VALUE'], "ACTIVE"=>"Y");
                                $rrsElement = CIBlockElement::GetList(Array('SORT'=>'ASC'), $arrrFilter, false, Array("nPageSize"=>1), $arrrSelect); 
                                if($arrElement = $rrsElement->GetNext()){                                        
                                        echo $arrElement['PROPERTY_REAL_PICTURE_VALUE'];
                                        $product['DETAIL_PICTURE'] = 'https://poisondrop.ru'.CFile::GetPath($arrElement['PROPERTY_REAL_PICTURE_VALUE']);
                                        $product['PICTURE'] = 'https://poisondrop.ru'.CFile::GetPath($arrElement['PROPERTY_REAL_PICTURE_VALUE']);
                                }
                               file_put_contents(__DIR__.'/testPic.log', var_export($product['PICTURE'],true),FILE_APPEND);
                            }
                //while ($product = $dbResProducts->GetNext()) {
...
$offer['_PROP_size'] =  $offer['PROPERTY_SIZE_RINGS_NAME'];
$offer['_PROP_manufacturer'] = $product['PROPERTY_DESIGNER_STR_NAME'];
    ...
    protected function BuildOffer($arOffer, $categories, $iblock, &$allCategories)
    {
        ...
        $offer .= "<param code=\"code\" name=\"Код товара\">" . $this->PrepareValue($arOffer["EXTERNAL_ID"]) . "</param>\n";
        $offer .= "<param code=\"composition\" name=\"Состав\">" . $this->PrepareValue($arOffer["COMPOSITION"]) . "</param>\n";
        if (!empty($arOffer["CML2_ARTICLE"]) && $arOffer["CML2_ARTICLE"]) {
                    $offer .= "<param code=\"article\">" . $this->PrepareValue($arOffer["CML2_ARTICLE"]) . "</param>\n";
                }
