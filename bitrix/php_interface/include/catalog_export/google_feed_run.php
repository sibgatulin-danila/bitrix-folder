...
if (empty($arRunErrors))
{
	...
	@fwrite($fp, "<link>http://".htmlspecialcharsbx($ar_iblock['SERVER_NAME'])."</link>\n");
	...
	if ('D' == $arCatalog['CATALOG_TYPE'] || 'O' == $arCatalog['CATALOG_TYPE'])
	{
		...
		while ($obElement = $res->GetNextElement())
		{
			...
			$str_AVAILABLE = ('Y' == $arAcc['CATALOG_AVAILABLE'] ? 'true' : 'false');
...
/*					$dbVAT = CCatalogProduct::GetVATInfo($arAcc['ID']);
			...
			$strTmpOff.= "<g:availability>".(stripos($str_TYPE.$str_AVAILABLE, 'true') === true ? 'preorder' : 'in stock')."</g:availability>\n";
			$strTmpOff.= "<link>http://".$ar_iblock['SERVER_NAME'].htmlspecialcharsbx($arAcc["~DETAIL_PAGE_URL"]).(strstr($arAcc['DETAIL_PAGE_URL'], '?') === false ? '?' : '&amp;')."r1=merchant</link>\n";

			$strTmpOff.= "<g:price>".$minPrice."</g:price>\n";
			...
			//newpic
			/*if(!empty($arAcc['PROPERTIES']['PHOTOGALLERY']['VALUE'])) {
				if(empty($arItem['PREVIEW_PICTURE']['SRC'])) {
				  if($albumFirst = CIBlockElement::GetList(array('SORT'=>'ASC'),array('IBLOCK_CODE'=>'photogallery','SECTION_ID'=>$arAcc['PROPERTIES']['PHOTOGALLERY']['VALUE']),false,array('nTopCount'=>1),array('PREVIEW_PICTURE'))->Fetch()) {
				    $arAcc['PREVIEW_PICTURE']['SRC'] = CFile::GetPath($albumFirst['PREVIEW_PICTURE']);
					$strFile = "http://".$ar_iblock['SERVER_NAME']."/".$arAcc['PREVIEW_PICTURE']['SRC'];
				    $strTmpOff.="<g:image_link>".$strFile."</g:image_link>\n";
				  }
				}
			}*/
			...
			foreach ($arYandexFields as $key)
			{
				...
				case 'name':
					...
					//$strTmpOff .= "<name>name2=".yandex_text2xml($arAcc["~NAME"], true)."</name>\n";
					break;
				case 'description':
					$strTmpOff .=
						"<description>".
		...
		while ($obItem = $rsItems->GetNextElement())
		{
			...
			if(!empty($arItem['PROPERTIES']['PHOTOGALLERY']['VALUE'])) {
				if(empty($arItem['PREVIEW_PICTURE'])) {
				  if($albumFirst = CIBlockElement::GetList(
				  		array('SORT'=>'ASC'),
				  		array('IBLOCK_CODE'=>'photogallery','SECTION_ID'=>$arItem['PROPERTIES']['PHOTOGALLERY']['VALUE']),
				  		false,array('nTopCount'=>1),array('PREVIEW_PICTURE'))
				  	->Fetch()) {
				    $arItem['PREVIEW_PICTURE'] = $albumFirst['PREVIEW_PICTURE'];
				  }
				}
			}
			...
			if (YANDEX_SKU_EXPORT_MIN_PRICE == $arSKUExport['SKU_EXPORT_COND'])
			{
				...
				while ($obOfferItem = $rsOfferItems->GetNextElement())
				{
					...
					if ($XML_DATA['PRICE'] > 0)
					{
						...
						if ($arPrice = $rsPrices->Fetch())
						{
							...
							$minPriceSale = CCatalogProduct::CountPriceWithDiscount($arPrice['PRICE'], $arPrice["CURRENCY"], $arDiscounts);
							$minPriceSaleRUR = CCurrencyRates::ConvertCurrency($arPrice['PRICE'], $arPrice["CURRENCY"], $RUR);

							//$minPriceSaleGroup = $arPrice['CATALOG_GROUP_ID'];
							//$minPriceCurrency = $arPrice["CURRENCY"];
							//$minPriceSaleRUR = CCurrencyRates::ConvertCurrency($arPrice['PRICE'], $arPrice["CURRENCY"], $RUR);
							/* 	sale end */
							if ($arOptimalPrice = CCatalogProduct::GetOptimalPrice(
					...
					else
					{
						...
						))
						{
							...
							$minPriceSale = $arPrice['DISCOUNT_PRICE'];
							$minPriceCurrency = $BASE_CURRENCY;
							$minPriceRUR = CCurrencyRates::ConvertCurrency($minPrice, $BASE_CURRENCY, $RUR);
							$minPriceGroup = $arPrice['PRICE']['CATALOG_GROUP_ID'];
						}
				...
				if (!empty($arCurrentOffer) && !empty($arCurrentPrice))
				{
					...
					$strOfferYandex .= "<g:availability>".(trim($arOfferItem['YANDEX_AVAILABLE']) != 'true' ? 'out of stock' : 'in stock')."</g:availability>\n";
					$strOfferYandex .= "<link>http://".$ar_iblock['SERVER_NAME'].htmlspecialcharsbx($arOfferItem["~DETAIL_PAGE_URL"]).(strstr($arOfferItem['DETAIL_PAGE_URL'], '?') === false ? '?' : '&amp;')."r1=merchant</link>\n";
					file_put_contents($_SERVER['DOCUMENT_ROOT'].'/available.txt', print_r($arOfferItem['YANDEX_AVAILABLE'], true), FILE_APPEND);
					file_put_contents($_SERVER['DOCUMENT_ROOT'].'/available.txt', "\n", FILE_APPEND);
					#$strOfferYandex .= "<g:price>".$minPrice."</g:price>\n";
					$strOfferYandex .= "<g:price>".$minPriceSale."</g:price>\n";
					#вывод
					#$strOfferYandex .= "<price_sale>".$minPriceSale."</price_sale>\n";
					...
					$pic_file = CFile::GetPath($arItem["PREVIEW_PICTURE"]);
					$pic = 'http://poisondrop.ru/'.$pic_file;
					if($pic_file=='') {
						continue;	
					}
					$strOfferYandex .= "<g:image_link>".$pic."</g:image_link>\n";
					...
					/*if (!empty($strFile) || !empty($arItem['YANDEX_PICT']))
					{
						...
						$strOfferYandex .= "<g:image_link>pic1</g:image_link>\n";
					}*/
					...
					foreach ($arYandexFields as $key)
					{
						...
						case 'name':
							...
							#$designer = '<![CDATA['.$arItem['PROPERTIES'][34]['~VALUE'].']]>';
							$designer = '';
							if ($arItem['PROPERTIES'][34]['~VALUE'] != '') {
								$designer = ', '.print_r($arItem['PROPERTIES'][34]['~VALUE'], true);
								$designer = htmlspecialchars($designer);
							}
							$name = $arItem['NAME'];
							//$strOfferYandex .= "<name>".yandex_text2xml($arOfferItem["~NAME"], true)."</name>\n";
							$strOfferYandex .= "<title>"./*$designer."|".*/yandex_text2xml($name, true).$designer."</title>\n";
							break;
						case 'description':
...
/*							$dbVAT = CCatalogProduct::GetVATInfo($arOfferItem['ID']);
					...
					$strOfferYandex = '';
					$strOfferYandex .= "<item id=\"".$arOfferItem["ID"]."\"".$str_TYPE." available=\"".$arOfferItem['YANDEX_AVAILABLE']."\">\n";
					$strOfferYandex .= "<g:id>".$arOfferItem["ID"]."</g:id>\n";
					$strOfferYandex .= "<link>http://".$ar_iblock['SERVER_NAME'].htmlspecialcharsbx($arOfferItem["~DETAIL_PAGE_URL"]).(strstr($arOfferItem['DETAIL_PAGE_URL'], '?') === false ? '?' : '&amp;')."r1=merchant</link>\n";
					...
					foreach ($arYandexFields as $key)
					{
						...
						case 'name':
							...
							$strOfferYandex .= "<title>".yandex_text2xml($arOfferItem["~NAME"], true)."</title>\n";
							//$strOfferYandex .= "<name>name3=".yandex_text2xml($arOfferItem["~NAME"], true)."</name>\n";
							break;
						case 'description':
							$strOfferYandex .= "<description>";
							if (strlen($arOfferItem['~PREVIEW_TEXT']) <= 0)
			...
			if ('X' == $arCatalog['CATALOG_TYPE'] && !$boolItemOffers)
			{
				...
				$str_AVAILABLE = ('Y' == $arItem['CATALOG_AVAILABLE'] ? 'true' : 'false');
...
/*						$dbVAT = CCatalogProduct::GetVATInfo($arItem['ID']);
				...
				$strOfferYandex.= "<g:condition>New</g:condition>\n";
				/*file_put_contents($_SERVER['DOCUMENT_ROOT'].'/available.txt', print_r($str_TYPE.$str_AVAILABLE, true), FILE_APPEND);
				file_put_contents($_SERVER['DOCUMENT_ROOT'].'/available.txt', "\n", FILE_APPEND);*/
				$strOfferYandex.= "<g:availability>".(trim($str_TYPE.$str_AVAILABLE) != 'true' ? 'out of stock' : 'in stock')."</g:availability>\n";
				$strOfferYandex.= "<link>http://".$ar_iblock['SERVER_NAME'].htmlspecialcharsbx($arItem["~DETAIL_PAGE_URL"]).(strstr($arItem['DETAIL_PAGE_URL'], '?') === false ? '?' : '&amp;')."r1=merchant</link>\n";
				...
				foreach ($arYandexFields as $key)
				{
					...
					case 'name':
						...
						$strValue = "<title>"/*.$designer.'|'*/.yandex_text2xml($arItem["~NAME"], true).$designer."</title>\n";
						break;
					case 'description':
						$strValue =
							"<description>".
