				...
				if (!empty($arCurrentOffer) && !empty($arCurrentPrice))
				{
					...
					foreach ($arYandexFields as $key)
					{
						...
						case 'name':
							...
							$strOfferYandex .= "<name>".$designer."|".yandex_text2xml($name, true)."|".$arItem['PROPERTIES'][13]['VALUE']."|".$minPriceSale."</name>\n";
							break;
						case 'description':
				...
				foreach ($arYandexFields as $key)
				{
					...
					case 'name':
						if (is_array($XML_DATA) && ($XML_DATA['TYPE'] == 'vendor.model' || $XML_DATA['TYPE'] == 'artist.title'))
							continue;
						$designer = '<![CDATA['.$arItem['PROPERTIES'][34]['~VALUE'].']]>';
						$strValue = "<name>".$designer.'|'.yandex_text2xml($arItem["~NAME"], true)."|".$arItem['PROPERTIES'][13]['VALUE']."|".$minPriceSale."</name>\n";
						break;
					case 'description':
						$strValue =
							"<description>".
