...
//Parameter was not passed to the component
        !isset($arParams["SKIP_SOURCE_CHECK"]) || !in_array($arParams["SKIP_SOURCE_CHECK"], array("Y", "N"))
) {
    //If default skip mode is not set
    if (
            isset($_GET["mode"]) && $_GET["mode"] !== "checkauth" && check_bitrix_sessid() //we got valid token from 1C
...
$bDesignMode = $APPLICATION->GetShowIncludeAreas() && !isset($_GET["mode"]) && is_object($USER) && $USER->IsAdmin();
if (!$bDesignMode) {
    if (!isset($_GET["mode"])) {
        return;
    }
...
if (
        !empty($_GET["filename"]) && is_string($_GET["filename"]) && strlen($DIR_NAME) > 0
) {
    ...
    $filename = trim(
            str_replace(
                    "\\", "/", trim(
                            preg_replace(
                                    "#^(/tmp/|upload/1c/webdata)#", "", $_GET["filename"]
                                    )
                            )
                    ),
            "/");
...
if ($_GET["mode"] == "checkauth" && $USER->IsAuthorized()) {
    if (
            (COption::GetOptionString("main", "use_session_id_ttl", "N") == "Y") && (COption::GetOptionInt("main", "session_id_ttl", 0) > 0) && !defined("BX_SESSION_ID_CHANGE")
    ) {
...
} elseif ((!$arParams["SKIP_SOURCE_CHECK"]) && (!check_bitrix_sessid())) { //Security checks are follow
...
} elseif ($_GET["mode"] == "init") { //Prepare exchange place
...
} elseif (($_GET["mode"] == "file") && $ABS_FILE_NAME) { //Receive files
    //Read http data
...
} elseif (($_GET["mode"] == "import") && $_SESSION["BX_CML2_IMPORT"]["zip"]) { //Unpack them if necessary
...
} elseif (($_GET["mode"] == "import") && $ABS_FILE_NAME) { //Step by step import
    ...
    if ($NS["STEP"] < 1) {
        ...
        $NS["STEP"] = 1;
        require($_SERVER["DOCUMENT_ROOT"] . "/import/parse.php");
    } elseif ($NS["STEP"] == 1) {
...
} elseif ($_GET["mode"] == "deactivate") { //Final step
    if ($_GET["timestamp"] > 0) {
...
if (!$bDesignMode) {
    ...
    <table class="data-table">
        <tr><td><?= GetMessage("CC_BCI1_IBLOCK_TYPE") ?></td><td><?= $arParams["IBLOCK_TYPE"] ?></td></tr>
        <tr><td><?= GetMessage("CC_BCI1_SITE_LIST") ?></td><td><?= implode(", ", $arParams["SITE_LIST"]) ?></td></tr>
        <tr><td><?= GetMessage("CC_BCI1_INTERVAL") ?></td><td><?= $arParams["INTERVAL"] ?></td></tr>
        <tr><td><?= GetMessage("CC_BCI1_ELEMENT_ACTION") ?></td><td><?= $arAction[$arParams["ELEMENT_ACTION"]] ?></td></tr>
        <tr><td><?= GetMessage("CC_BCI1_SECTION_ACTION") ?></td><td><?= $arAction[$arParams["SECTION_ACTION"]] ?></td></tr>
        <tr><td><?= GetMessage("CC_BCI1_FILE_SIZE_LIMIT") ?></td><td><?= $arParams["FILE_SIZE_LIMIT"] ?></td></tr>
        <tr><td><?= GetMessage("CC_BCI1_USE_CRC") ?></td><td><?= $arParams["USE_CRC"] ? GetMessage("MAIN_YES") : GetMessage("MAIN_NO") ?></td></tr>
        <tr><td><?= GetMessage("CC_BCI1_USE_ZIP") ?></td><td><?= $arParams["USE_ZIP"] ? GetMessage("MAIN_YES") : GetMessage("MAIN_NO") ?></td></tr>
        <tr><td><?= GetMessage("CC_BCI1_USE_IBLOCK_PICTURE_SETTINGS") ?></td><td><?= $arParams["USE_IBLOCK_PICTURE_SETTINGS"] ? GetMessage("MAIN_YES") : GetMessage("MAIN_NO") ?></td></tr>
        <?php if (!$arParams["USE_IBLOCK_PICTURE_SETTINGS"]): ?>
            <tr><td><?= GetMessage("CC_BCI1_GENERATE_PREVIEW") ?></td><td><?= $arParams["GENERATE_PREVIEW"] ? GetMessage("MAIN_YES") . " " . $arParams["PREVIEW_WIDTH"] . "x" . $arParams["PREVIEW_HEIGHT"] : GetMessage("MAIN_NO") ?></td></tr>
            <tr><td><?= GetMessage("CC_BCI1_DETAIL_RESIZE") ?></td><td><?= $arParams["DETAIL_RESIZE"] ? GetMessage("MAIN_YES") . " " . $arParams["DETAIL_WIDTH"] . "x" . $arParams["DETAIL_HEIGHT"] : GetMessage("MAIN_NO") ?></td></tr>
        <?php endif ?>
        <tr><td><?= GetMessage("CC_BCI1_TRANSLIT_ON_ADD") ?></td><td><?= $arParams["TRANSLIT_ON_ADD"] ? GetMessage("MAIN_YES") : GetMessage("MAIN_NO") ?></td></tr>
        <tr><td><?= GetMessage("CC_BCI1_TRANSLIT_ON_UPDATE") ?></td><td><?= $arParams["TRANSLIT_ON_UPDATE"] ? GetMessage("MAIN_YES") : GetMessage("MAIN_NO") ?></td></tr>
        <?php if ($arParams["TRANSLIT_ON_ADD"] || $arParams["TRANSLIT_ON_UPDATE"]): ?>
            <tr><td><?= GetMessage("CC_BCI1_TRANSLIT_MAX_LEN") ?></td><td><?= $arParams["TRANSLIT_MAX_LEN"] ?></td></tr>
            <tr><td><?= GetMessage("CC_BCI1_TRANSLIT_CHANGE_CASE") ?></td>
                <td>
                    <?php
            ...
            <tr><td><?= GetMessage("CC_BCI1_TRANSLIT_REPLACE_SPACE") ?></td><td><?= $arParams["TRANSLIT_REPLACE_SPACE"] ?></td></tr>
            <tr><td><?= GetMessage("CC_BCI1_TRANSLIT_REPLACE_OTHER") ?></td><td><?= $arParams["TRANSLIT_REPLACE_OTHER"] ?></td></tr>
            <tr><td><?= GetMessage("CC_BCI1_TRANSLIT_DELETE_REPEAT_REPLACE") ?></td><td><?= $arParams["TRANSLIT_DELETE_REPEAT_REPLACE"] ? GetMessage("MAIN_YES") : GetMessage("MAIN_NO") ?></td></tr>
        <?php endif ?>
        <tr><td><?= GetMessage("CC_BCI1_USE_OFFERS") ?></td><td><?= $arParams["USE_OFFERS"] ? GetMessage("MAIN_YES") : GetMessage("MAIN_NO") ?></td></tr>
        <tr><td><?= GetMessage("CC_BCI1_FORCE_OFFERS") ?></td><td><?= $arParams["FORCE_OFFERS"] ? GetMessage("MAIN_YES") : GetMessage("MAIN_NO") ?></td></tr>
        <tr><td><?= GetMessage("CC_BCI1_DISABLE_CHANGE_PRICE_NAME") ?></td><td><?= $arParams["DISABLE_CHANGE_PRICE_NAME"] ? GetMessage("MAIN_YES") : GetMessage("MAIN_NO") ?></td></tr>
    </table>
<?php } ?>
