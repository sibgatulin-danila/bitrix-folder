(function(window) {
	...
	window.BXPhotoList.prototype = {
		...
		AddElement: function(Item)
		{
			...
			pParams.appendChild(BX.create("label", {props: {className: 'photo-al-ed-label-top hidden', 'for': 'bxph_desc_' + Item.id}, text: this.MESS.albumDesc}));
			var pDesc = pParams.appendChild(BX.create("textarea", {props: {className: 'photo-al-ed-width hidden', id: 'bxph_desc_' + Item.id, name: inpName + '[desc]'}}));
		...
		Rotate: function(id, type)
		{
			...
			if (Item)
			{
				...
				if (BX.browser.IsIE() && BX.browser.IsDoctype())  // TODO: fix bogus ie rotation paggings
					Item.pThumb.firstChild.className = BX.browser.IsIE9() ? 'photo-rotate-ie9-' + Item.ange : 'photo-rotate-' + Item.ange;
				else
