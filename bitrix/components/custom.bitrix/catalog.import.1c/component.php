...
elseif (($_GET["mode"] == "file") && $ABS_FILE_NAME)
{
    ...
    /*file_put_contents($_SERVER['DOCUMENT_ROOT'].'/test/ost.xml', $DATA);
    $xml = simplexml_load_string($DATA);
    file_put_contents($_SERVER['DOCUMENT_ROOT'].'/test/ost2.txt', print_r($xml, 1));
    foreach ($xml as $current) {
        file_put_contents($_SERVER['DOCUMENT_ROOT'].'/test/ost3.txt', print_r($xml, 1), FILE_APPEND);
    }
    $xml->ПакетПредложений->ТипыЦен->ТипЦены[0]->Валюта = 'RUB';
    unset($xml->ПакетПредложений->ТипыЦен->ТипЦены[1]);
    unset($xml->ПакетПредложений->ТипыЦен->ТипЦены[1]);
    foreach ($xml->ПакетПредложений->Предложения->Предложение as $order) {
        unset($order->Цены->Цена[1]);
        unset($order->Цены->Цена[1]);
    }
    $newFile = $_SERVER['DOCUMENT_ROOT'].'/upload/update_ost.xml';
    $xml->asXml($newFile);
	...
	file_put_contents($_SERVER['DOCUMENT_ROOT'].'/test/ost_fn.txt',$ABS_FILE_NAME);
