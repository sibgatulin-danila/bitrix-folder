...
file_put_contents($_SERVER['DOCUMENT_ROOT'].'/upload/ms.txt', "get \n", FILE_APPEND);
file_put_contents($_SERVER['DOCUMENT_ROOT'].'/upload/ms.txt', print_r($_GET,1)."\n", FILE_APPEND);
file_put_contents($_SERVER['DOCUMENT_ROOT'].'/upload/ms.txt', "request \n", FILE_APPEND);
file_put_contents($_SERVER['DOCUMENT_ROOT'].'/upload/ms.txt', print_r($_REQUEST,1)."\n", FILE_APPEND);
...
else
{
	...
	{
		/////////
		file_put_contents($_SERVER['DOCUMENT_ROOT'].'/upload/ms.txt', print_r($_SESSION["BX_CML2_EXPORT"],1), FILE_APPEND);
		if($ABS_FILE_NAME)
		{
			...
			elseif(isset($GLOBALS["HTTP_RAW_POST_DATA"]))
				...
				$DATA = false;
			//file_put_contents($_SERVER['DOCUMENT_ROOT'].'/upload/ex.txt', print_r($DATA, 1), FILE_APPEND);

			if(isset($DATA) && $DATA !== false)
			{
				CheckDirPath($ABS_FILE_NAME);
				//file_put_contents($_SERVER['DOCUMENT_ROOT'].'/upload/ex.txt', print_r($ABS_FILE_NAME, 1), FILE_APPEND);
	...
	{
		file_put_contents($_SERVER['DOCUMENT_ROOT'].'/upload/ms.txt', 'new_version', FILE_APPEND);
		if($ABS_FILE_NAME)
		{
			if(function_exists("file_get_contents"))
				$DATA = file_get_contents("php://input");
