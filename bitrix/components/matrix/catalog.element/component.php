	...
	if (($action == "ADD2BASKET" || $action == "BUY" || $action == "SUBSCRIBE_PRODUCT") && $productID > 0)
	{
		if (CModule::IncludeModule("sale") && CModule::IncludeModule("catalog") && CModule::IncludeModule('iblock'))
		{
			...
			if ($arItem = $rsItems->Fetch())
			{
				...
				{
					if (!empty($arParams["PRODUCT_PROPERTIES"]) && !empty($_REQUEST[$arParams["PRODUCT_PROPS_VARIABLE"]]['SIZE']))
					{
						...
						{
						    if(!empty($_REQUEST[$arParams["PRODUCT_PROPS_VARIABLE"]]['SIZE']))
						      $product_properties[]=array('NAME'=>'Размер','CODE'=>'SIZE','VALUE'=>$_REQUEST[$arParams["PRODUCT_PROPS_VARIABLE"]]['SIZE'][0]);
			...
			//$arRewriteFields["CAN_BUY"] = "N";
			//$arRewriteFields['CALLBACK_FUNC'] = 'MyBasketCallback';
			if(in_array($productID,array(7356,7357,7500))) {				
			   if(!empty($_REQUEST['text']) && !empty($_REQUEST['f']) && !empty($_REQUEST['c']) && !empty($_REQUEST['ct'])) {
			     
			     $product_properties[]=array('NAME'=>'Текст','CODE'=>'NP_TEXT','VALUE'=>$_REQUEST['text']);
			     $product_properties[]=array('NAME'=>'Цвет','CODE'=>'NP_COLOR','VALUE'=>$_REQUEST['c']);
			     $product_properties[]=array('NAME'=>'Шрифт','CODE'=>'NP_FONT','VALUE'=>$_REQUEST['f']);
			     $product_properties[]=array('NAME'=>'Вид плетения','CODE'=>'NP_CHAIN','VALUE'=>$_REQUEST['ct']=='c1'?'якорное':'панцырьное');
			//     $product_properties[]=array('NAME'=>'Длина цепочки','CODE'=>'NP_CHAIN_LEN','VALUE'=>$_REQUEST['lt']);
