...
function checkUser($userLogin,$USER) {
    $rsUser = CUser::GetByLogin($userLogin);
    $rsUserByEmail = CUser::GetList(
        ($by = "id"),
        ($order = "asc"),
        array(
            "EMAIL" => $userLogin
    ...
    $obUserByEmail = $rsUserByEmail->Fetch();
    if ($rsUser) {
        $arUser = $rsUser->Fetch();
...
parse_str($_REQUEST['POST'], $arRequest);
global $USER;
if ($arRequest) {
    $userLogin = $arRequest['ORDER_PROP_6'];
    if (!$USER->IsAuthorized()) {
        checkUser($userLogin, $USER);
    }
}
...
$arResult["AUTH"]["new_user_registration_email_confirmation"] = "Y";
/*$arResult["AUTH"]["new_user_registration"] = "Y";*/
	...
	if(IntVal($_REQUEST["ORDER_ID"]) <= 0)
	{
		...
		if($arUserResult["CONFIRM_ORDER"] == "Y" && empty($arResult["ERROR"]))
		{
			...
			if(!$USER->IsAuthorized() && $arParams["ALLOW_AUTO_REGISTER"] == "Y")
			{
				...
				if(strlen($arUserResult["USER_EMAIL"]) > 0)
				{
                    $NEW_PHONE = $arUserResult["ORDER_PROP"]["18"];
					$NEW_LOGIN = $arUserResult["USER_EMAIL"];
					//$NEW_LOGIN = $arUserResult["ORDER_PROP"]["18"];
