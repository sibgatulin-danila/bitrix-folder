...
if($this->StartResultCache(false, array($arrFilter, ($arParams["CACHE_GROUPS"]==="N"? false: $USER->GetGroups()), $arNavigation)))
{
	...
	if(!$bSectionFound)
	{
		if(!in_array($arParams["SECTION_CODE"],array('sale','new'))) {
			$this->AbortResultCache();
			ShowError(GetMessage("CATALOG_SECTION_NOT_FOUND"));
			@define("ERROR_404", "Y");
			if($arParams["SET_STATUS_404"]==="Y")
	...
	if($arParams['WISHLIST']=='Y')
		$arFilter['!ID'] = array(4181);
	else
		$arFilter['!ID'] = array(4181,7353,11087,11727,24948);
	//EXECUTE

	$fullFilter = array_merge($arrFilter, $arFilter);
	$APPLICATION->IncludeComponent("pd:rr.widget",".default",array('TYPE' => 'SEARCH', 'SHOW_TEMPLATE' => 'N'), false);
	if ($arParams['INCLUDERR'] == 'Y') {
		global $rrSearch;
		$fullFilter = array(
			'ID' => $rrSearch,
			'!ID' => array(4181,7353,11087,11727,24948)
		);
	}
	//file_put_contents($_SERVER['DOCUMENT_ROOT'].'/test/fullfilter.txt', print_r($fullFilter, 1));
	$rsElements = CIBlockElement::GetList($arSort, $fullFilter, false, $arNavParams, $arSelect);
	if(empty($arParams['PREF']))
	...
	if (!empty($arResult["ELEMENTS"]) && $bGetProperties)
	{
		...
		foreach ($arResult["ITEMS"] as &$arItem)
		{
			...
			if(!empty($arItem['PROPERTIES']['PHOTOGALLERY']['VALUE'])) {
				if($lastPhoto = CIBlockElement::GetList(array('SORT'=>'DESC'),array('IBLOCK_CODE'=>'photogallery','SECTION_ID'=>$arItem['PROPERTIES']['PHOTOGALLERY']['VALUE']),false,array('nTopCount'=>1),array('PREVIEW_PICTURE'))->Fetch()) {
					$arItem['LAST_IMG'] = CFile::GetPath($lastPhoto['PREVIEW_PICTURE']);
				}

				if(empty($arItem['PREVIEW_PICTURE']['SRC'])) {
					if($albumFirst = CIBlockElement::GetList(array('SORT'=>'ASC'),array('IBLOCK_CODE'=>'photogallery','SECTION_ID'=>$arItem['PROPERTIES']['PHOTOGALLERY']['VALUE']),false,array('nTopCount'=>1),array('PREVIEW_PICTURE'))->Fetch()) {
		...
		if(!empty($arOffers))
		{
			...
			foreach($arOffers as $arOffer)
			{
				...
				if (isset($arElementLink[$arOffer["LINK_ELEMENT_ID"]]))
				{
					...
					//$arResult["ITEMS"][$arOffer["LINK_ELEMENT_ID"]]["PRICES"] = $arOffer['PRICES'];	
