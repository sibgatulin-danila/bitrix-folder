...
<?endif;*/?>
    ...
    $autoJSON = json_encode($cityAutocomplete);
    ?>
    <script>
        $(document).ready(function() {
            var availableTags = $.parseJSON('<?=$autoJSON;?>');
            $( "[name=ORDER_PROP_2]" ).autocomplete({
                source: availableTags,
                minLength: 1,
                autoFocus: true,
                appendTo: '#city_line',
                position: { my : "left top", at: "left bottom", of: '#city_line' },
                change: function( event, ui ) {
                    /*var hidden_id = ui.item.hidden_id;
                    if (parseInt(hidden_id)> 0) {
                        $('[name=ORDER_PROP_2]').val(hidden_id);
                        $('.city_error').hide();
                    }*/

                },
                response: function(event, ui) {
                   /* var city_id = ui.content[0].hidden_id;
                    if (city_id > 0) {
                        $('[name=ORDER_PROP_2]').val(city_id);
                    } else{
                        $('[name=ORDER_PROP_2]').val(0);
                    }*/
                }
            });
        });
