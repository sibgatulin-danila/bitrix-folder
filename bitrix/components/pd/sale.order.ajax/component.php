...
if ($USER->IsAuthorized() || $arParams["ALLOW_AUTO_REGISTER"] == "Y" )
{
	if(IntVal($_REQUEST["ORDER_ID"]) <= 0)
	{
		...
		if((!empty($arResult['POST_PARAMS']) && $arResult['POST_PARAMS']['profile_change']=='Y') || $checkUserProfiles->SelectedRowsCount()==0) {	
                  $loc =  CPoisonUtils::getCityByIP();
		  $arUserResult["ORDER_PROP"][2] = $loc['LOCATION_CODE'];
		}		
		 
		if($_SERVER["REQUEST_METHOD"] == "POST" && isset($arResult['POST_PARAMS']["confirmorder"]) && ($arParams["DELIVERY_NO_SESSION"] == "N" || check_bitrix_sessid()) || !empty($arUserResult["ORDER_PROP"][2])) {
			...
			while ($arOrderProps = $dbOrderProps->GetNext())
			{
				...
				//{
					...
					if($arOrderProps["ID"]==11) {
					//  $arUserResult["TELEPHONE"] =  $curVal;	  
					}
