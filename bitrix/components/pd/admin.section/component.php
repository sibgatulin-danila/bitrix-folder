...
if($this->StartResultCache(false, array($arrFilter, ($arParams["CACHE_GROUPS"]==="N"? false: $USER->GetGroups()), $arNavigation)))
{
	...
	while($obElement = $rsElements->GetNextElement())
	{
		...
		$catalogItem = CCatalogProduct::GetByID($arItem['ID']);
		...
		if(!empty($arItem["PROPERTIES"]['THUMB']['VALUE']))
		 $thumb = CFile::GetPath($arItem["PROPERTIES"]['THUMB']['VALUE']);
		else {

		   $thumb = CPoisonUtils::getFirstThumb($arItem['ID']);
		 }
