...
$APPLICATION->AddHeadScript('/js/jquery.autosize.min.js');
	...
	if ($bAllowAccess)
	{
		...
		if (check_bitrix_sessid() && (!empty($_REQUEST["iblock_submit"]) || !empty($_REQUEST["iblock_apply"])))
		{
			...
			$arResult["SEF_URL"] = $SEF_URL;
                 ...
                 if ($arParams["ID"] > 0)  {
			   if($currElement =  CIBlockElement::GetList(array(),array('IBLOCK_ID'=> $arParams["IBLOCK_ID"],'ID'=> $arParams["ID"]),false,false)->GetNext()){
				  $_REQUEST['PROPERTY']['NAME'] = array($currElement['NAME']);
			   }				
			}
			...
			if (count($arResult["ERRORS"]) == 0)
			{
				...
				if ($arParams["ID"] > 0)
				{
                    ...
                    $ELEMENT_ID = $_GET['CODE'];  // код элемента
                    $PROPERTY_CODE = "LINKLABEL";  // код свойства
                    $PROPERTY_VALUE = $_POST['linklabel'];  // значение свойства

                    // Установим новое значение для данного свойства данного элемента
                    CIBlockElement::SetPropertyValuesEx($ELEMENT_ID, false, array($PROPERTY_CODE => $PROPERTY_VALUE));

					$ELEMENT_ID = $_GET['CODE'];  // код элемента
                    $PROPERTY_CODE = "LOGO_MAG";  // код свойства
                    $PROPERTY_VALUE = $_POST['logo'];  // значение свойства
					...
					if($arProperties['ACTIVE'][0]=='Y')
					  $arUpdateValues['ACTIVE']='Y';
					 else
					  $arUpdateValues['ACTIVE']='N';
					  
					  
					  $rsOffers = CIBlockElement::GetList(array(),array('IBLOCK_ID'=>11,'PROPERTY_CML2_LINK'=>$arParams["ID"]),false,false,array('ID'));
					  
					  if($rsOffers->SelectedRowsCount()>0) {

							if(intval($arUpdateValues['PROPERTY_VALUES'][27][0])>0)  {  
								$rsEnumValue = CIBlockProperty::GetPropertyEnum("DISCOUNT", Array(), Array("IBLOCK_ID"=>$arParams["IBLOCK_ID"], "ID"=>$arUpdateValues['PROPERTY_VALUES'][27][0]));					   					   
								if($arEnumValue = $rsEnumValue->GetNext()) {								     
								    $rsEnumValuesOffer = CIBlockProperty::GetPropertyEnum("DISCOUNT", Array(), Array("IBLOCK_ID"=>11, "VALUE"=>$arEnumValue['VALUE']));
								    if($arEnumValuesOffer  = $rsEnumValuesOffer->GetNext()) {
								       while($offer=$rsOffers->Fetch())  {						
											CIBlockElement::SetPropertyValuesEx($offer['ID'],11,array('DISCOUNT'=>$arEnumValuesOffer['ID']));
								       }
								    }
								...
								while($offer=$rsOffers->Fetch())  {			
							        CIBlockElement::SetPropertyValuesEx($offer['ID'],11,array('DISCOUNT'=>76));
							    }
						    }
					  }
					...
					if (!$res = $oElement->Update($arParams["ID"], $arUpdateValues, $bWorkflowIncluded, true, $arParams["RESIZE_IMAGES"]))
					{
					    ...
					    $priceFields = Array("PRODUCT_ID" => $arParams["ID"], "CATALOG_GROUP_ID" => 1, "PRICE" => intval($arUpdateValues['PRICE']),  "CURRENCY" => "RUB");
					    ...
					    if ($arr = $res->Fetch()) {
						  ...
						  CPrice::Add($priceFields);
					       ...
					       $res = CCatalogProduct::GetList(array(), array("ID" => $arParams['ID']));
						if ($arr = $res->Fetch()) {
						 
						 $quantFields = array("ID" => $arParams["ID"], "QUANTITY"=>$arr['QUANTITY'],'WEIGHT'=>$arr['WEIGHT']);     							
						 CCatalogProduct::Update($arr['ID'], $quantFields);					
					       } else {
						 $quantFields = array("ID" => $arParams["ID"], "QUANTITY"=>0,'WEIGHT'=>0);     
						 CCatalogProduct::Add($quantFields);	
					       }
					   }
			...
			if (count($arResult["ERRORS"]) == 0)
			{
				...
				if($sAction=='ADD') {
				 LocalRedirect($sRedirectUrl.'&edit=Y&CODE='.$arParams['ID']);
  				 exit();
				} else
				 {
					LocalRedirect($sRedirectUrl.'&edit=Y&CODE='.$arParams['ID']);
		...
		//prepare logo magazine for form

		$arSelect = Array("ID", "IBLOCK_ID", "NAME", "DATE_ACTIVE_FROM","PROPERTY_LINKLABEL","PREVIEW_PICTURE","DETAIL_PICTURE");
		$arFilter = Array("IBLOCK_ID"=>26, "ACTIVE_DATE"=>"Y", "ACTIVE"=>"Y");
		$res = CIBlockElement::GetList(Array(), $arFilter, false, Array("nPageSize"=>50), $arSelect);
		while($ob = $res->Fetch()){
		    //$arFields = $ob->GetFields();
	    	$arResult['LOGO_MAGAZINE'][] = $ob;
/*	    	echo '<pre>';
	    	print_r($arResult['LOGO_MAGAZINE']);
	    	echo '</pre>';*/
		}
		...
		if ($arParams["ID"] > 0)
		{
			...
			//$arResult["ELEMENT"] = $arElement;
                       	...
                       	$res = CPrice::GetList(array(), array("PRODUCT_ID" =>  $arElement["ID"]));			
			if ($arr = $res->Fetch()) {				
			  $arResult["ELEMENT"]['PRICE']	 = $arr['PRICE'];
			}
						
			$res = CCatalogProduct::GetList(array(), array("ID" => $arParams['ID']));
			if ($arr = $res->Fetch()) {				
			  $arResult["ELEMENT"]['QUANTITY'] = $arr['QUANTITY'];
			}		
			...
			if(!empty($arResult["ELEMENT_PROPERTIES"][16][0]['VALUE'])) {
			  if($album = CIBlockSection::GetByID($arResult["ELEMENT_PROPERTIES"][16][0]['VALUE'])->GetNext()) {
				$arResult['ALBUM']= $album; 
			  }
			}
