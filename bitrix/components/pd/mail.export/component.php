...
if($this->StartResultCache(false, array($arParams["CACHE_GROUPS"]==="N"? false: $USER->GetGroups(), $arrFilter)))
{
	...
	$rsSections = CIBlockSection::GetList(array(), array('IBLOCK_ID'=>$arParams["IBLOCK_ID"],'ACTIVE'=>'Y'));
	...
	$rsElements = CIBlockElement::GetList($arSort, array_merge($arFilter, $arrFilter), false, false, $arSelect);
	...
	while($obElement = $rsElements->GetNextElement()) {
		...
		if(empty($arElement['PREVIEW_PICTURE']))
                  $arElement['PREVIEW_PICTURE'] = 'http://poisondrop.ru/'.CPoisonUtils::getFirstImg($arElement['ID']);
