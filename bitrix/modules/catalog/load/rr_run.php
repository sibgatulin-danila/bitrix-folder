...
function getGalleryIMG($id) {
	...
	$db = CIBlockElement::GetList(
		...
		false,
		array('nTopCount' => 1),
		array('IBLOCK_ID', 'ID', 'PROPERTY_REAL_PICTURE')
	);
	
	if ($rs = $db->Fetch()) {
		$image = 'http://poisondrop.ru'.GetSizeDetail(array('ITEM'=>$rs['PROPERTY_REAL_PICTURE_VALUE'], 'H'=>600, 'W'=>600));
	}
...
if (empty($arRunErrors))
{
	...
	file_put_contents($_SERVER['DOCUMENT_ROOT'].'/test/catalogtype.txt', print_r($arCatalog, 1), FILE_APPEND);
	if ('D' == $arCatalog['CATALOG_TYPE'] || 'O' == $arCatalog['CATALOG_TYPE'])
	{
		$arSelect = array("ID", "LID", "IBLOCK_ID", "IBLOCK_SECTION_ID", "NAME", "PREVIEW_PICTURE", "PREVIEW_TEXT", "PREVIEW_TEXT_TYPE", "DETAIL_PICTURE", "LANG_DIR", "DETAIL_PAGE_URL", "PROPERTY_PHOTOGALLERY");
		...
		file_put_contents($_SERVER['DOCUMENT_ROOT'].'/test/3rr.txt', print_r($filter, 1));
...
/*					$dbVAT = CCatalogProduct::GetVATInfo($arAcc['ID']);
			...
			$strTmpOff.= "<offer test=\"1\" id=\"".$arAcc["ID"]."\"".$str_TYPE.$str_AVAILABLE.">\n";
			...
			if ($arItem['PROPERTY_PHOTOGALLERY_VALUE']) {
				$strFile = getGalleryIMG($arItem['PROPERTY_PHOTOGALLERY_VALUE']);
			}
			file_put_contents($_SERVER['DOCUMENT_ROOT'].'/test/item.txt', print_r($strFile, 1), FILE_APPEND);
			//die;
			$strTmpOff.="<picture>".$strFile."</picture>\n";
	...
	{
		$arOfferSelect = array("ID", "LID", "IBLOCK_ID", "NAME", "PREVIEW_PICTURE", "PREVIEW_TEXT", "PREVIEW_TEXT_TYPE", "DETAIL_PICTURE", "DETAIL_PAGE_URL", "PROPERTY_PHOTOGALLERY");
		...
		$arSelect = array("ID", "LID", "IBLOCK_ID", "IBLOCK_SECTION_ID", "NAME", "PREVIEW_PICTURE", "PREVIEW_TEXT", "PREVIEW_TEXT_TYPE", "DETAIL_PICTURE", "DETAIL_PAGE_URL", "PROPERTY_PHOTOGALLERY");
		$arFilter = array("IBLOCK_ID" => $IBLOCK_ID, '!ID' => array(4181,24948));
		/*if (!$bAllSections && !empty($arSectionIDs))
		{
		...
		$arFilter["ACTIVE"] = "Y";
		$arFilter["PROPERTY_IN_ARCHIVE"] = false;
		...
		file_put_contents($_SERVER['DOCUMENT_ROOT'].'/test/1rr.txt', print_r($arFilter, 1));
		$cnt = 0;
		...
		while ($obItem = $rsItems->GetNextElement())
		{
			...
			if ($arItem['ID'] == 17836) {
				file_put_contents($_SERVER['DOCUMENT_ROOT'].'/test/1rr.txt', print_r($arItem, 1));
			}
			$inArchive = $arItem['PROPERTIES']['IN_ARCHIVE']['VALUE'];
			...
			file_put_contents($_SERVER['DOCUMENT_ROOT'].'/test/2rr.txt', print_r($arOfferFilter, 1));
...
/*							$dbVAT = CCatalogProduct::GetVATInfo($arOfferItem['ID']);
				...
				if (!empty($arCurrentOffer) && !empty($arCurrentPrice))
				{
					...
					{
						$arProduct['QUANTITY'] = doubleval($arProduct['QUANTITY']);
						if ((0 >= $arProduct['QUANTITY'] && 'Y' == $arProduct['QUANTITY_TRACE'] && 'N' == $arProduct['CAN_BUY_ZERO']) || $inArchive == 'yes')
                    ...
                    file_put_contents($_SERVER['DOCUMENT_ROOT'].'/upload/xml.txt', "\ntest2\n", FILE_APPEND);
                    file_put_contents($_SERVER['DOCUMENT_ROOT'].'/upload/xml.txt', print_r($arPrint, 1), FILE_APPEND);
					$strOfferYandex .= "<offer "./*"test=\"2\"".*/" id=\"".$arItem["ID"]/*$arOfferItem["ID"]*/."\"".$str_TYPE." available=\"".$arOfferItem['YANDEX_AVAILABLE']."\">\n";
...
/*							$dbVAT = CCatalogProduct::GetVATInfo($arOfferItem['ID']);
					...
					$strOfferYandex = '';
					$strOfferYandex .= "<offer test=\"3\" id=\"".$arOfferItem["ID"]."\"".$str_TYPE." available=\"".$arOfferItem['YANDEX_AVAILABLE']."\">\n";
					$strOfferYandex .= "<url>http://".$ar_iblock['SERVER_NAME'].htmlspecialcharsbx($arOfferItem["~DETAIL_PAGE_URL"]).(strstr($arOfferItem['DETAIL_PAGE_URL'], '?') === false ? '?' : '&amp;')."r1=<?echo \$strReferer1; ?>&amp;r2=<?echo \$strReferer2; ?></url>\n";
...
/*						$dbVAT = CCatalogProduct::GetVATInfo($arItem['ID']);
                ...
                file_put_contents($_SERVER['DOCUMENT_ROOT'].'/upload/xml.txt', "\ntest4\n", FILE_APPEND);
                file_put_contents($_SERVER['DOCUMENT_ROOT'].'/upload/xml.txt', print_r($arPrint, 1), FILE_APPEND);
				$strOfferYandex.= "<offer "./*"test=\"4\"".*/" id=\"".$arItem["ID"]."\"".$str_TYPE.$str_AVAILABLE.">\n";
