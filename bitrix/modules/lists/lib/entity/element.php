...
class Element implements Controllable, Errorable
{
	...
	private function isEnabledBizproc()
	{
		return (Loader::includeModule("bizproc") &&
			\CBPRuntime::isFeatureEnabled() && ($this->params["BIZPROC"] === "Y"));
	}
	...
	private function getBizprocData()
	{
		...
		$documentId = ($this->params["ELEMENT_ID"] ? \BizProcDocument::getDocumentComplexId(
			$this->params["IBLOCK_TYPE_ID"], $this->params["ELEMENT_ID"]) : null);
		$documentStates = \CBPDocument::getDocumentStates($documentType, $documentId);

		global $USER;
		$userId = $USER->getID();
		$currentUserGroups = $USER->getUserGroupArray();
		if (!$this->params["ELEMENT_ID"] || $this->params["CREATED_BY"] == $userId)
		{
		...
		if ($this->params["ELEMENT_ID"])
		{
			$canWrite = \CBPDocument::canUserOperateDocument(
				...
				\BizProcDocument::getDocumentComplexId($this->params["IBLOCK_TYPE_ID"], $this->params["ELEMENT_ID"]),
